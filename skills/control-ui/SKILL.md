...
You have a web-based control UI running at `http://localhost:3000/setup`. Use it to manage your runtime environment.
...
2. Tell the user to fill it in:
   > I need a BRAVE_API_KEY for web search. I've added a placeholder — you can set it at http://localhost:3000/setup (Envars tab), or edit `/data/.env` directly.
...
Config lives at `/data/.openclaw/gogcli/`. If the user needs to connect or reconnect Google, direct them to http://localhost:3000/setup (Google Workspace section).
